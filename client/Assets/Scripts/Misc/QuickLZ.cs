...
public static class QuickLZ
{
    ...
    private static int headerLen(byte[] source, int srcOffset)
    {
        return ((source[srcOffset + 0] & 2) == 2) ? 9 : 3;
    }

    public static int sizeDecompressed(byte[] source, int srcOffset)
    {
        if (headerLen(source, srcOffset) == 9)
            return source[srcOffset + 5] | (source[srcOffset + 6] << 8) | (source[srcOffset + 7] << 16) | (source[srcOffset + 8] << 24);
        else
            return source[srcOffset + 2];
    }

    public static int sizeCompressed(byte[] source, int srcOffset)
    {
        if (headerLen(source, srcOffset) == 9)
            return source[srcOffset + 1] | (source[srcOffset + 2] << 8) | (source[srcOffset + 3] << 16) | (source[srcOffset + 4] << 24);
        else
            return source[srcOffset + 1];
    }
	...
	static int[] hashtable = new int[4096];
	static byte[] hash_counter = new byte[4096];

	public static int decompress(byte[] source, int srcOffset, byte[] destination)
	{
		int level;
        int size = sizeDecompressed(source, srcOffset);
        int src = srcOffset + headerLen(source, srcOffset);
        int dst = 0;
        uint cword_val = 1;
        ...
        {
			System.Array.Copy(source, srcOffset + headerLen(source, srcOffset), destination, 0, size);
            return size;
        }
        ...
        for (; ; )
        {
            ...
            if ((cword_val & 1) == 1)
            {
                ...
                {
					if (dst + i >= destination.Length || offset2 + i >= destination.Length)
					{
						//DebugLoggerging.LogError("destination" + destination.Length);
						return 0;
					}
