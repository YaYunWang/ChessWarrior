...
public class EventDelegate
{
	...
	void Cache ()
	{
		...
		if (mCachedCallback == null || (mCachedCallback.Target as MonoBehaviour) != mTarget || GetMethodName(mCachedCallback) != mMethodName)
		{
			if (mTarget != null && !string.IsNullOrEmpty(mMethodName))
			{
				...
				{
					DebugLogger.LogError("Failed to bind " + type + "." + mMethodName + "\n" +  ex.Message);
					return;
				}

				if (mMethod == null)
				{
					DebugLogger.LogError("Could not find method '" + mMethodName + "' on " + mTarget.GetType(), mTarget);
					return;
				}
				
				if (mMethod.ReturnType != typeof(void))
				{
					DebugLogger.LogError(mTarget.GetType() + "." + mMethodName + " must have a 'void' return type.", mTarget);
					return;
				}
	...
	static public void Add (List<EventDelegate> list, EventDelegate ev, bool oneShot)
	{
		...
		else DebugLogger.LogWarning("Attempting to add a callback to a list that's null");
	}
